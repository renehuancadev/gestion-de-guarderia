<x-layout title="Editar Niño">
    <form action="{{ route('ninos.update', $nino->id) }}" method="POST" class="max-w-sm mx-auto">
        @csrf
        @method('PUT')

        <div class="mb-5">
            <label for="nombre" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nombre del Niño</label>
            <input type="text" id="nombre" name="nombre" value="{{ old('nombre', $nino->nombre) }}"
        ...
        <div class="mb-5">
            ...
            <input type="date" id="fecha_nacimiento" name="fecha_nacimiento" value="{{ old('fecha_nacimiento', $nino->fecha_nacimiento) }}"
                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg 
                          focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 
                          dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 
                          dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" 
        ...
        <div class="mb-5">
            ...
            <select id="tutor_id" name="tutor_id" 
                ...
                @foreach($tutores as $tutor)
                    <option value="{{ $tutor->id }}" {{ old('tutor_id', $nino->tutor_id) == $tutor->id ? 'selected' : '' }}>
                        {{ $tutor->nombre }}
                    </option>
                @endforeach
            </select>
